...
export const ExerciseSelector = ({ recommendedExercises = [] }) => {
  ...
  useEffect(() => {
    webcamRef.current = document.getElementById('webcam-section');
  }, []);
  ...
  const handleSkipVideo = () => {
    ...
    if (window.innerWidth < 768 && webcamRef.current) {
  ...
  return (
    <div className="mb-4 sm:mb-6">
      <div className="flex items-center justify-between mb-3 sm:mb-4">
        <h2 className="text-base sm:text-lg font-bold text-[#e8f0ff]" style={{ fontFamily: "'Syne', sans-serif" }}>
          Select Exercise
        </h2>
        {recommendedExercises.length > 0 && (
          <span className="text-xs text-[#00e5ff] px-2 sm:px-3 py-1 bg-[#00e5ff]/10 border border-[#00e5ff]/30 rounded-full"
      ...
      {Object.entries(categories).map(([category, exercises]) => {
        ...
        return (
          <div key={category} className="mb-4 sm:mb-5">
